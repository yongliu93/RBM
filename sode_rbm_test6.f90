    ...
    !end module try
      ...
      !do K=6,6
       ...
       do i=1,1
           ...
           epsilon=1.0E-07!1.0E-02-i*1.0E-03
...
!    ns=nend
    ...
    do l=1,s_eps(pos)%index(0)
        ...
        f0=f0+sqrt(real(alpha(pos,s_eps(pos)%index(l))))* &
         u0(s_alpha(pos)%index(l))*Mk(0,s_eps(pos)%index(l))*dt
    end do
    ...
    uA(num_M,1)=unum(1)
    uA(num_M,2:numstep)=unum(2:numstep)-(1+dt)*unum(1:numstep-1)
    
    AAu(1:numstep-1,num_M)=uA(num_M,1:numstep-1)-(1+dt)*uA(num_M,2:numstep)
    AAu(numstep,num_M)=uA(num_M,numstep)
    ...
    do i=0, M-1
    Mu(i,2:numstep,num_M)=Mk(1:numstep-1,i)*unum(1:numstep-1)
    ...
    AA(numstep,numstep-1)=-(1+dt)
    AA(numstep,numstep)=1.
