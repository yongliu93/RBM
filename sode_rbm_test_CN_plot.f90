      ...
      real, allocatable, dimension(:,:,:) :: Mu,uAMu,Mu1,Mu2!,uMAu
    ...
    !end module try
      ...
      write(*,"(5ES12.2)") epsilon,error2,t2-t1,real(num_M),timenorbm
      !call outpdata
      ...
      !pause
       ...
       call deletememory
        ...
        allocate(u1(0:NPC-1,1:numstep))
        
        allocate(AA(1:numstep,1:numstep))
    ...
    !u1=0.
     ...
     allocate(Mu1(0:M-1,1:numstep,1:maxbasisnum),Mu2(0:M-1,1:numstep,1:maxbasisnum))
...
!      end subroutine
     ...
     unum(0)=1.0
      ...
      deallocate(u_num,Ic,alpha,s_eps,s_alpha,I_cindex,u1)
     
      
      deallocate(f_LHS,r_Basis,C_basis,u_rbm,unum,basisnum,u0,frb)
      deallocate(pickup)
      deallocate(uAAu,Mu,uA,uAMu,AAu,Mu1,Mu2)
      deallocate(uAAu_i)
      deallocate(ipiv,Mk)
    ...
    !tn=(0)*dt
        ...
        do l=1,s_eps(pos)%index(0)
         ...
         (u0(s_alpha(pos)%index(l))*Mk(0,s_eps(pos)%index(l))+&
          u1(s_alpha(pos)%index(l),1)*Mk(1,s_eps(pos)%index(l)))
        end do
        f_LHS(1)=0.5*dt*f_LHS(1)+u0(pos)+0.5*dt*u0(pos)
    ...
    !     sum((Mu(s_eps(pos)%index(l),i,1:num_M)+Mu2(s_eps(pos)%index(l),i,1:num_M))*C_basis(s_alpha(pos)%index(l),0:num_M-1))
    !    end do
    !    f_LHS(i)=0.5*dt*f_LHS(i)
    !    
    !end do
    ...
    do i=2,numstep
        ...
        do l=1,s_eps(pos)%index(0)
          f_LHS(i)=f_LHS(i)+sqrt(real(alpha(pos,s_eps(pos)%index(l))))* &
         (u1(s_alpha(pos)%index(l),i-1)*Mk(i-1,s_eps(pos)%index(l))+&
          u1(s_alpha(pos)%index(l),i)*Mk(i,s_eps(pos)%index(l)))
        end do
        f_LHS(i)=0.5*dt*f_LHS(i)
    ...
    !tn=(0)*dt
        !call LegendrePoly(tn,0.,endt,M-1,basis)
        do l=1,s_eps(pos)%index(0)
            tmp=sum(C_basis(s_alpha(pos)%index(l),0:num_M-1)*r_basis(1,0:num_M-1))
          f_LHS(1)=f_LHS(1)+sqrt(real(alpha(pos,s_eps(pos)%index(l))))* &
         (u0(s_alpha(pos)%index(l))*Mk(0,s_eps(pos)%index(l))+&
         tmp*Mk(1,s_eps(pos)%index(l)))
        end do
        f_LHS(1)=0.5*dt*f_LHS(1)+u0(pos)+0.5*dt*u0(pos)
    ...
    do i=2,numstep
        ...
        !call LegendrePoly(tn,0.,endt,M-1,basis)
        do l=1,s_eps(pos)%index(0)
            tmp1=sum(C_basis(s_alpha(pos)%index(l),0:num_M-1)*r_basis(i-1,0:num_M-1))
            tmp2=sum(C_basis(s_alpha(pos)%index(l),0:num_M-1)*r_basis(i,0:num_M-1))
          f_LHS(i)=f_LHS(i)+sqrt(real(alpha(pos,s_eps(pos)%index(l))))* &
         (tmp1*Mk(i-1,s_eps(pos)%index(l))+&
          tmp2*Mk(i,s_eps(pos)%index(l)))
        end do
        f_LHS(i)=0.5*dt*f_LHS(i)
    ...
    frb(1:num_M)=uA(1:num_M,1)*(u0(pos)+0.5*dt*u0(pos))
    
    do l=1,s_eps(pos)%index(0)
        
        frb(1:num_M)=frb(1:num_M)+0.5*uA(1:num_M,1)*dt*sqrt(real(alpha(pos,s_eps(pos)%index(l))))*Mk(0,s_eps(pos)%index(l))*u0(s_alpha(pos)%index(l))
        do i=1,num_M
...
!        end do
     ...
     !if (resrb<epsilon) then
      !if(pickup(pos) .eq. 1) then 
         ...
         !write(*,*) resrb
         !do i=0,num_M-1
         !u2(pos,1:numstep)=u2(pos,1:numstep)+C_basis(pos,i)*r_basis(1:numstep,i)
         !end do
         ...
         !u2(pos,1:numstep)=unum(1:numstep)
...
!    ns=nend
    ...
    f0=(u0(pos)+0.5*dt*u0(pos))
    
    do l=1,s_eps(pos)%index(0)
        
        f0=f0+sqrt(real(alpha(pos,s_eps(pos)%index(l))))* &
         u0(s_alpha(pos)%index(l))*Mk(0,s_eps(pos)%index(l))*dt/2.
    end do
    
    resrb=sum((tmp2(1:nrank)-res_Q(1,1:nrank)*f0)**2)
    ...
    !AAu(numstep,num_M+1)=unum(numstep)-(1+dt)*unum(numstep-1)
        ...
        do j=1,num_M
            ...
            tmp1=sum(unum(1:numstep)*r_basis(1:numstep,j-1))
            tmpvec2(1:numstep)=unum(1:numstep)-tmp1*r_basis(1:numstep,j-1)
            ...
            if (tmp2 < 1.0E-17) then
                ...
                unum(1:numstep)=tmpvec2(1:numstep)/tmp2
                num_Mup=1
    ...
    uA(num_M,1)=a*unum(1)
    uA(num_M,2:numstep)=a*unum(2:numstep)-b*unum(1:numstep-1)
    
    AAu(1:numstep-1,num_M)=a*uA(num_M,1:numstep-1)-b*uA(num_M,2:numstep)
    AAu(numstep,num_M)=a*uA(num_M,numstep)
    
    do i=1,num_M
    uAAu(num_M,i)=sum(uA(num_M,1:numstep)*uA(i,1:numstep))
    uAAu(i,num_M)=uAAu(num_M,i)
    end do
    
    uAAu(num_M,num_M)=sum(uA(num_M,1:numstep)*uA(num_M,1:numstep))
    
    !uA(num_M,1:numstep)=Au(1:numstep,num_M)
    
    
    do i=0, M-1
    Mu(i,1,num_M)=Mk(1,i)*unum(1)
    Mu(i,2:numstep,num_M)=Mk(1:numstep-1,i)*unum(1:numstep-1)+Mk(2:numstep,i)*unum(2:numstep)
    
    do j=1,num_M
    uAMu(i,num_M,j)=sum(uA(num_M,1:numstep)*Mu(i,1:numstep,j))!sum(unum(2:numstep)*Mk(1:numstep-1,i)*r_basis(2:numstep,j-1))-(1+dt)*sum(r_basis(3:numstep,j-1)*unum(2:numstep-1)*Mk(1:numstep-2,i))
    !uMAu(i,j,num_M)=uAMu(i,num_M,j)
    uAMu(i,j,num_M)=sum(uA(j,1:numstep)*Mu(i,1:numstep,num_M))!sum(unum(2:numstep)*Mk(1:numstep-1,i)*r_basis(2:numstep,j-1))-(1+dt)*sum(r_basis(2:numstep-1,j-1)*unum(3:numstep)*Mk(1:numstep-2,i))
    !uMAu(i,num_M,j)=uAMu(i,j,num_M)
    end do
    
    uAMu(i,num_M,num_M)=sum(uA(num_M,1:numstep)*Mu(i,1:numstep,num_M))!sum(unum(2:numstep)*Mk(1:numstep-1,i)*unum(2:numstep))-(1+dt)*sum(unum(3:numstep)*unum(2:numstep-1)*Mk(1:numstep-2,i))
    ...
    do i=-1,M-1
        ...
        if (i .eq. -1) then
            tmpvec1(1:numstep)=uA(num_M,1:numstep)
        else
    ...
    use comdata
      ...
      unum(0)=1.0
      do i=1,numstep
          unum(i)=(unum(i-1)+0.5*dt*(unum(i-1)+1.0)+0.5*dt)/(1-0.5*dt)
      end do
      ...
      !u_rbm(pos,1:numstep)=unum(1:numstep)
          ...
          unum(1)=f_LHS(1)/(1-0.5*dt)
          do i=2,numstep
          unum(i)=(f_LHS(i)+(1.+0.5*dt)*unum(i-1))/(1-0.5*dt)
          end do
    ...
    AA=0.
    do i=1,numstep-1
    AA(i,i)=(1-0.5*dt)**2+(1+0.5*dt)**2
    AA(i,i+1)=-(1+0.5*dt)*(1-0.5*dt)
    AA(i+1,i)=AA(i,i+1)
    end do
    AA(numstep,numstep-1)=-(1+0.5*dt)*(1-0.5*dt)
    AA(numstep,numstep)=(1.-0.5*dt)**2
    ...
    call run_ori_rbm(0)
    !u2(0,1:numstep)=unum(1:numstep)
    
    rnorm=sqrt(sum(unum(1:numstep)**2))
    unum(1:numstep)=unum(1:numstep)/rnorm
    ...
    do i=0, M-1
    Mu1(i,2:numstep,1)=Mk(1:numstep-1,i)*unum(1:numstep-1)
    Mu2(i,1:numstep-1,1)=Mu1(i,2:numstep,1)
    Mu2(i,numstep,1)=Mk(numstep,i)*unum(numstep)
    
    Mu(i,1:numstep,1)=Mu1(i,1:numstep,1)+Mu2(i,1:numstep,1)
    ...
    use comdata
    real, allocatable, dimension(:) :: u_rbmtmp
    
    allocate(u_rbmtmp(1:numstep))
    ...
    do i=0,NPC-1
        ...
        u_rbmtmp(1:numstep)=u_rbmtmp(1:numstep)+C_basis(i,j)*r_basis(1:numstep,j)
        end do
        error2=max(error2,sqrt(sum(abs(u1(i,1:numstep)-u_rbmtmp(1:numstep))**2)))
    end do
    ...
    rbmem2=sum(u_rbm(1:NPC-1)*u_rbm(1:NPC-1))
    write(*,*) 'E[0]=',u_rbm(0),'E[1]=',rbmem2
    write(*,*)
    
    write(2018,*) 'E[0]=',u_rbm(0),'E[1]=',rbmem2
    write(2018,*)
    
    apem1=u1(0,numstep)
    apem2=sum(u1(1:NPC-1,numstep)**2)!sum(u_num(1:NPC-1)*u_num(1:NPC-1))
    !em2=22.3982744180594         
    error=abs(apem2-rbmem2)
    write(*,*) 'error0_truth=', abs(u_rbm(0)-apem1),'errror1_truth=',error
    write(*,*)
    
    write(2018,*) 'error0_truth=', abs(u_rbm(0)-apem1),'errror1_truth=',error
    write(2018,*)
    
    em1=2.0*exp(1.0)-1.0
    em2=(7.0*exp(3.0)-6.0*exp(1.0)+2.0)/3.0
    em2=em2-em1*em1
    write(*,*) 'rbmem[0]=',u_rbm(0),'apem[1]=',rbmem2
    write(*,*) 'em[0]=',em1,'em[1]=',em2
    write(*,*)
    write(*,*) 'error0_exact=', abs(u_rbm(0)-em1),'errror1_exact=',abs(rbmem2-em2)
    
    
    write(2018,*) 'rbmem[0]=',u_rbm(0),'apem[1]=',rbmem2
    write(2018,*) 'em[0]=',em1,'em[1]=',em2
    write(2018,*)
    write(2018,*) 'error0_exact=', abs(u_rbm(0)-em1),'errror1_exact=',abs(rbmem2-em2)
